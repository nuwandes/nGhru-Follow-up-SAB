...
import org.nghru_bd.ghru.event.AxivityRxBus
import org.nghru_bd.ghru.util.LocaleManager
import org.nghru_bd.ghru.vo.Axivity
...
class ActivityTrackerActivity : AppCompatActivity(), HasSupportFragmentInjector {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        Log.wtf("TRACKER_ACTIVITY","DATA- " + data)

        when (requestCode) {
            ACTIVITY_TRACKER_REQUEST_CODE -> {
                //Timber.d("session_id" + data?.getStringExtra("sesstion_id"))
                Timber.d("start_time" + data?.getStringExtra("start_time"))
                Timber.d("end_time" + data?.getStringExtra("end_time"))
                Timber.d("serial_number" + data?.getIntExtra("serial_number", 0).toString())
                Timber.d("session_id" + data?.getStringExtra("session_id").toString())

                val axivity = Axivity(
                    sessionId = data?.getStringExtra("session_id").toString(),
