package org.nghru_pk.ghru.ui.participantlist.measurementlist
...
import org.nghru_pk.ghru.*
import org.nghru_pk.ghru.binding.FragmentDataBindingComponent
import org.nghru_pk.ghru.databinding.MeasurementListFragmentBinding
import org.nghru_pk.ghru.db.MemberTypeConverters
import org.nghru_pk.ghru.di.Injectable
import org.nghru_pk.ghru.ui.participantlist.measurementlist.completevisitcompleted.VisitCompletedDialogFragment
import org.nghru_pk.ghru.ui.participantlist.measurementlist.completevisitwarning.VisitWarningDialogFragment
import org.nghru_pk.ghru.util.autoCleared
import org.nghru_pk.ghru.util.fromJson
import org.nghru_pk.ghru.util.singleClick
import org.nghru_pk.ghru.vo.MeasurementListItem
import org.nghru_pk.ghru.vo.ParticipantListItem
import org.nghru_pk.ghru.vo.ParticipantStation
...
//                startActivity(intent)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val measurementAdapter = MeasurementListAdapter(dataBindingComponent, appExecutors, isConsent) { measurementListItem ->
            ...
            {
                navigateToStationActivity(participant!!.participant_id, BodyMeasurementsActivity())
            }

            if (measurementListItem.id == 2)
            {
                navigateToStationActivity(participant!!.participant_id, BloodPressureActivity())
            }

            if (measurementListItem.id == 3)
            {
                navigateToStationActivity(participant!!.participant_id, SpirometryActivity())
            }

            if (measurementListItem.id == 4) {
                navigateToStationActivity(participant!!.participant_id, BloodTestActivity())
            }

            if (measurementListItem.id == 5) {
                navigateToStationActivity(participant!!.participant_id, WebViewActivity())
            }

            if (measurementListItem.id == 6) {
                navigateToStationActivity(participant!!.participant_id, SampleCollectionActivity())
            }

            if (measurementListItem.id == 7) {
                navigateToStationActivity(participant!!.participant_id, IntakeActivity())
            }

            if (measurementListItem.id == 8) {
                if (isInProgress!!)
                {
                    navigateToStationActivity(participant!!.participant_id, CovidQuestionnaireNewActivity())
                }
                else
                {
                    navigateToStationActivity(participant!!.participant_id, CovidQuestionnaireActivity())
                }
            }
        }
...
//        })
    ...
    private fun navigateToStationActivity(participantId: String?, nextActivity: Activity )
    {
        // navigate to next activity
        val json1: String = MemberTypeConverters.gson.toJson(participant)
...
//        measurementListViewModel.setParticipantRequest(participantId)
//
//        measurementListViewModel.getParticipantRequest.observe(this, Observer { participantResource ->
//
...
//
//                val intent = Intent(activity, nextActivity::class.java)
//                intent.putExtra("ParticipantID", participantRequest?.screeningId)
//                startActivity(intent)
//
//            }
